/**
 * @file L0x03.h
 * @brief Simple Remote Lingo 0x03 Processor Header for esPod.
 */
...
/**
 * @brief Processor class for Simple Remote Lingo (0x03) commands.
 */
class L0x03
{
public:
    static void processLingo(esPod *esp, const uint8_t *byteArray, uint32_t len);
    static void _0x00_iPodAck(esPod *esp, IPOD_ACK_CODE ackCode, uint8_t cmdID);
    static void _0x00_iPodAck(esPod *esp, IPOD_ACK_CODE ackCode, uint8_t cmdID, uint32_t numField);
    ...
    static void _0x10_RetPlayStatus(esPod *esp, uint8_t playState, uint32_t trackIndex, uint32_t trackTotMs, uint32_t trackPosMs);
    static void _0x13_RetIndexedPlayingTrackInfo(esPod *esp, uint8_t trackInfoType, char *trackInfoChars);
    ...
    static void _0x1B_RetPowerBatteryState(esPod *esp, uint8_t powerBatteryState);
    static void _0x1D_RetSoundCheckState(esPod *esp, uint8_t soundCheckState);
