...
1. **System Power-On & Initialization (`app_main`)**:
   ...
   - `usb_espod_bridge_task` launched on **Core 1** (`CONFIG_USB_ESPOD_BRIDGE_TASK_PRIORITY`, stack `CONFIG_USB_ESPOD_BRIDGE_TASK_STACK_SIZE`).
...
1. **Host-Initiated Pause**:
   ...
   - `espod::pause()` invokes `_playStatusHandler(PB_CMD_PAUSE)` -> calls `bt_a2dp_sink_pause()`.
...
| **Core 1 (APP_CPU)** | **`usb_espod_bridge_task`** | **10** (`CONFIG_USB_ESPOD_BRIDGE_TASK_PRIORITY`) | `vTaskDelay(pdMS_TO_TICKS(5))` | **Over-Polling Prevention**: 5ms yield loop prevents CPU starvation if USB is idle/unplugged. Stack size: `CONFIG_USB_ESPOD_BRIDGE_TASK_STACK_SIZE` (4096). |
